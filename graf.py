...
def priprav(mereni, index_hodnoty, barva, jednotka, delitel=1, desetin=0):
    ...
    hodnoty = [radek[index_hodnoty] / delitel for radek in mereni]
    casy = [radek[0] for radek in mereni]

    # Osa y vždy začíná na nule. U výkonu i procent to tak patří -
    # useknutá osa opticky zveličuje rozdíly a je to klasický klam.
    y_max = _hezke_maximum(max(hodnoty))
    ...
    # Výplň pod čárou: stejná cesta, ale dole uzavřená k ose x.
    dno = OKRAJ_NAHORE + PLOCHA_VYSKA
    vypln = (f"M {body[0][0]:.1f},{dno:.1f} "
             + " ".join(f"L {x:.1f},{y:.1f}" for x, y in body)
             + f" L {body[-1][0]:.1f},{dno:.1f} Z")

    # --- Popisky osy y: nula, půlka, maximum ---
    y_popisky = []
    for podil in (0, 0.5, 1):
        hodnota = y_max * podil
        y_popisky.append({
            "y": OKRAJ_NAHORE + PLOCHA_VYSKA - podil * PLOCHA_VYSKA,
            "text": f"{hodnota:.{desetin}f}",
        })
    ...
    tecky = [
        {"x": x, "y": y,
         "popis": f"{_cas_na_popisek(casy[i])} — {hodnoty[i]:.{desetin}f} {jednotka}"}
    ...
    return {
        ...
        "pocet": len(hodnoty),
        "posledni": f"{hodnoty[-1]:.{desetin}f}",
        "maximum": f"{max(hodnoty):.{desetin}f}",
    }
